...
ifeq ($(COMPILE_IN_HOW_MANY_STEPS),1)
	...
	$(CXX) $(NAME_SRC) $(CXXFLAGS) $(MY_LINKING_FLAGS) $(OutputMark) $@
	@echo "end compiling and linking in one step: $@"
else ifeq ($(COMPILE_IN_HOW_MANY_STEPS),2)
     $(NAME_OUT):$(NAME_OBJ)
	@echo "start linking: $@."
	$(LD) $^ $(LDFLAGS) $(MY_LINKING_FLAGS) $(OutputMark) $@
	@echo "end linking: $@."
     $(NAME_OBJ):$(NAME_SRC) $(NAME_HEA)
	@echo "start compiling: $@."
	$(CXX) -c $< $(CXXFLAGS) $(OutputMark) $@
	@echo "end compiling: $@."
endif
...
But it can also be done in two step, the first one being the linking:
...
where "$^" is a shortcut for all the elements on the right, which in this case is only "$(NAME_OBJ)". For the linking we need to link to the shared libraries we need and tell it if it's shared object or not, so we need both elements in $(MY_LINKING_FLAGS). So for Helper it will look like this:
